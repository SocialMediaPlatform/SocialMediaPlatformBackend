...
public interface ConversationRepository extends JpaRepository<Conversation, Long> {
    ...
    @Query(value = "SELECT conversation_id FROM user_conversation " +
            "WHERE conversation_id IN (SELECT conversation_id FROM user_conversation WHERE user_id = :userId) " +
            "GROUP BY conversation_id HAVING COUNT(DISTINCT user_id) > 2", nativeQuery = true)
    List<Long> findGroupConversationIdsByUserId(@Param("userId") Long userId);

    @Query(value = "SELECT COUNT(*) > 0 FROM user_conversation WHERE user_id = :userId AND conversation_id = :conversationId", nativeQuery = true)
    boolean existsByUserIdAndConversationId(@Param("userId") Long userId, @Param("conversationId") Long conversationId);
}